...
exports.execute = function () {
  ...
  browserSync.watch(path.join(config.serve.src.length === 0 ? config.html.dest : config.serve.src, '**/*.html')).on('change', browserSync.reload)

  // Listen to change events on JS and reload
  browserSync.watch(path.join(config.serve.src.length === 0 ? config.scripts.dest : config.serve.src, '**/*.js')).on('change', browserSync.reload)

  // Provide a callback to capture ALL events to CSS
  // files - then filter for 'change' and reload all
  // css files on the page.
  browserSync.watch(path.join(config.serve.src.length === 0 ? config.styles.dest : config.serve.src, '**/*.css'), function (event, file) {
    if (event === 'change') {
